package com.alessiodp.oreannouncer.addons.external;
...
public class MetricsHandler {
	...
	public MetricsHandler(OreAnnouncer instance) {
		plugin = instance;
		registerMetrics();
	}
	...
	private void registerMetrics () {
		...
		metrics.addCustomChart(new Metrics.SimplePie("use_randomizer") {
			...
			public String getValue() {
				String ret = "Disabled";
				if (ConfigMain.COORDINATES_ENABLE && ConfigMain.COORDINATES_RANDOM_ENABLE)
					ret = "Enabled";
				return ret;
			}
		});
	...
	public static class Metrics {
		...
		static {
			...
			if (MetricsHandler.class.getPackage().getName().equals(defaultPackage) || MetricsHandler.class.getPackage().getName().equals(examplePackage)) {
				throw new IllegalStateException("bStats MetricsHandler class has not been relocated correctly!");
			}
		}
		...
		public Metrics(JavaPlugin plugin) {
			...
			if (config.getBoolean("enabled", true)) {
				boolean found = false;
				// Search for all other bStats MetricsHandler classes to see if we are the first one
		...
		private void submitData() {
			...
			// Search for all other bStats MetricsHandler classes to get their plugin data
			for (Class<?> service : Bukkit.getServicesManager().getKnownServices()) {
				try {
					service.getField("B_STATS_VERSION"); // Our identifier :)
				} catch (NoSuchFieldException ignored) {
